...
bool ParseConvert::LoadFBX(const std::string &fileName, std::vector<StaticMesh> &meshes) {//finish mtl load
	this->clearAll();
	std::map <std::string, std::unique_ptr<Materials>>::iterator matIter(materials.end());
	...
	if (lRootNode) {
		auto meshesCount = lRootNode->GetChildCount();
		meshes.resize(meshesCount);
		LclTranslation.resize(meshesCount);
		for (int i = 0; i < meshesCount; i++) {
			childNode = lRootNode->GetChild(i);
			mesh = childNode->GetMesh();
			meshes[i].SetName(childNode->GetName());
			meshes[i].SetTranslation(static_cast<float>(childNode->LclTranslation.Get()[0]), static_cast<float>(childNode->LclTranslation.Get()[1]), static_cast<float>(childNode->LclTranslation.Get()[2]));
			meshes[i].SetRotationAngles(static_cast<float>(childNode->LclRotation.Get()[0]), static_cast<float>(childNode->LclRotation.Get()[1]), static_cast<float>(childNode->LclRotation.Get()[2]));
			meshes[i].SetScale(static_cast<float>(childNode->LclScaling.Get()[0]), static_cast<float>(childNode->LclScaling.Get()[1]), static_cast<float>(childNode->LclScaling.Get()[2]));
				...
				if (normalEl) {
					...
					for (int polyCounter = 0; polyCounter < mesh->GetPolygonCount(); polyCounter++) {
						for (int j(0); j < 3; j++)
						{
							...
							auto findResult = std::find(meshes[i].vertex.begin(), meshes[i].vertex.end(), Vertex(this->verteñis[indecisBuffer[vertexCounter]], PCFLOAT3(static_cast<float>(normal[0]), static_cast<float>(normal[1]), static_cast<float>(normal[2])), PCFLOAT2(static_cast<float>(fbxTexCoord[0]), static_cast<float>(fbxTexCoord[1]))));
							
							if (findResult == meshes[i].vertex.end()) { //have not such vertex
								meshes[i].vertex.push_back(Vertex(this->verteñis[indecisBuffer[vertexCounter]], PCFLOAT3(static_cast<float>(normal[0]), static_cast<float>(normal[1]), static_cast<float>(normal[2])), PCFLOAT2(static_cast<float>(fbxTexCoord[0]), static_cast<float>(fbxTexCoord[1]))));// add new triangle
								meshes[i].indecis.push_back(meshes[i].vertex.size() - 1);
							}
							else {//already have vertex
								meshes[i].indecis.push_back(findResult - meshes[i].vertex.begin());
							}
							vertexCounter++;
						}
					}
...
void ParseConvert::Out() {
	...
	for (const auto &element : LclTranslation) {
		out << element.translation[0] << ',' << element.translation[1] << ',' << element.translation[2] << '\n';
	}
